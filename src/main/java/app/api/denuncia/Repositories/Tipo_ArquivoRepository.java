...
@Repository
@Transactional
public interface Tipo_ArquivoRepository extends JpaRepository<Tipo_ArquivoModel, Integer> {

    Tipo_ArquivoModel findById(@Param("id") int id);

    Tipo_ArquivoModel findByNome(@Param("nome") String nome);

    List<Tipo_ArquivoModel> findAllByEstado(@Param("estado") int estado);

    Tipo_ArquivoModel findByIdAndEstado(@Param("id") int id, @Param("estado") int estado);

    @Query(value = "select * from tipo_arquivo where id!=:id and nome=:nome", nativeQuery = true)
    Tipo_ArquivoModel validarTipoArquivo(@Param("id") int id, @Param("nome") String nome);
